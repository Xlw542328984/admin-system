...
<script>
import { getCate,getParams,add,edit,editParams,remove,save} from '../../network/params'
export default {
  data () {
    return {
      // 商品分类列表
  ...
  methods: {
    ...
    async getParamsData () {
      ...
      const { data: res } = await getParams(this.getCateId,{params: { sel: this.activeTabsName }})
      if (res.meta.status !== 200) {
    ...
    editParams () {
      this.$refs.editFromRef.validate(async valid => {
        ...
        const { data: res } = await editParams(
          this.getCateId,this.editFrom.attr_id,
          {
