...
<script>
import { getCate,getParentCate,add,remove,show,edit } from '../../network/cate'
export default {
  data () {
    return {
      // 商品分类数据
  ...
  methods: {
    ...
    async getParentCateList () {
      const { data: res } = await getParentCate({
        params: { type: 2 }
      })
      if (res.meta.status !== 200) {
    ...
    addCate () {
      this.$refs.addCateFormRef.validate(async valid => {
        if (!valid) return
        const { data: res } = await add(this.addCateForm)
    ...
    async removeCate (id) {
      ...
      const { data: res } = await remove('categories/' + id)
    ...
    async showEditCateDialog (id) {
      const { data: res } = await show('categories/' + id)
    ...
    eidtCate () {
      this.$refs.editCateFormRef.validate(async valid => {
        if (!valid) return
        const { data: res } = await edit('categories/' + this.editCateForm.cat_id,
          {
