...
<script>
import { getRoles,removeRight,add,showSetRight,showEdit,deleteRoles,editRole,allRight} from '../../network/power'
export default {
  data () {
    return {
      // 所有角色列表
  ...
  methods: {
    ...
    async removeRightById (role, rightId) {
      ...
      const { data: res } = await removeRight({role, rightId})
      if (res.meta.status !== 200) {
    ...
    async showSetRightDialog (role) {
      this.roleId = role.id
      // 获取角色的所有权限
      const { data: res } = await showSetRight()
      if (res.meta.status !== 200) {
    ...
    addRoles () {
      this.$refs.addRoleFormRef.validate(async valid => {
        if (!valid) return
        const { data: res } = await add(this.addRoleForm)
    ...
    async showEditDialog (id) {
      const { data: res } = await showEdit(id)
    ...
    async allotRights () {
      ...
      // console.log(roleId);
      const { data: res } = await allRight(this.roleId, { rids: idStr })
      console.log(res);
